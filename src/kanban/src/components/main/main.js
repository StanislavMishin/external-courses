...
export const updateStorageFromLocal = () => {
  localStorage.setItem('boardColumns', JSON.stringify(boardColumns));
};

export const updateLocalFromStorage = () => {
...
export function randomNumberForIdTasks(min, max) {
  return (Math.random() * (max - min) + min).toFixed(6);
}

export const repaintBoardFromLocal = () => {
  const mainContent = document.querySelector('.blockFlexRow');
